...
class quizaccess_numattempts extends quiz_access_rule_base {
    ...
    public function prevent_new_attempt($numprevattempts, $lastattempt) {
        ...
        if ($numprevattempts >= $this->quiz->attempts) {
          ...
          $course = $DB->get_record('course', array('id'=>(int)$this->quiz->course), '*', MUST_EXIST);
          $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'begateway'), '*', MUST_EXIST);

          return get_string('nomoreattempts', 'quiz') . $plugin->enrol_page_hook($instance, array( 'buymoreattempts' => true, 'quiz_id' => $this->quiz->id));
        }
        return false;
    }
